...
import { ArrowLeft, ChevronDown, Eye, Image, Plus, Save, Search, Send, Upload, X } from "lucide-react";
...
import {
  ...
  DIFF_COLORS, SECTIONS, SECTION_LABELS, uid, type TypeInfo,
...
function CreateQuestionWorkspace() {
  ...
  const [section, setSection] = useState<typeof SECTIONS[number]>("type");
  const [title, setTitle] = useState("");
  const [text, setText] = useState("");
  const [instructions, setInstructions] = useState("");
  const [answerData, setAnswerData] = useState<AnswerData | undefined>(createDefaultAnswerData("mcq"));
  ...
  const [acLinks, setAcLinks] = useState<QuestionAcademicLink[]>([]);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[200px_1fr_240px] gap-4">
        ...
        <div className="space-y-3">
          ...
          <Card className="border bg-card p-4 space-y-2"><Button className="w-full rounded-xl gradient-brand border-0 text-white text-xs h-9 gap-1.5" onClick={() => handleSave(true)}><Send className="h-3.5 w-3.5" />Publish</Button><Button variant="outline" className="w-full rounded-xl text-xs h-9 gap-1.5" onClick={() => handleSave(false)}><Save className="h-3.5 w-3.5" />Save{mode === "edit" ? "" : " Draft"}</Button><Button asChild variant="ghost" className="w-full rounded-xl text-xs h-9"><Link to="/teacher/questions">Cancel</Link></Button></Card>
        </div>
      </div>
    </div>
  );
