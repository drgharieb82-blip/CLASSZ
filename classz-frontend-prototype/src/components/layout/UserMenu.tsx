...
import { Settings, LogOut, User, Palette, Languages, Bell, ChevronsUpDown } from "lucide-react";
import {
    ...
    <DropdownMenu>
      <DropdownMenuTrigger className="flex items-center gap-2 rounded-xl border bg-card p-1.5 pr-2.5 text-sm transition-colors hover:bg-accent">
        <Avatar className="h-7 w-7">
          <AvatarFallback className="gradient-brand text-xs font-bold text-white">ME</AvatarFallback>
        </Avatar>
      ...
      <DropdownMenuContent align="end" className="w-60 rounded-xl">
        <DropdownMenuLabel>
          ...
          <p className="text-xs font-normal text-muted-foreground">alex@classz.io</p>
        </DropdownMenuLabel>
        ...
        <DropdownMenuSeparator />
        <DropdownMenuItem asChild><Link to="/login" className="gap-2 text-destructive"><LogOut className="h-4 w-4" /> Log out</Link></DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
}
