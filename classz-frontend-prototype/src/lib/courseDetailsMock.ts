...
export function getCourseDetails(courseId: string): CourseDetails | null {
  ...
  return {
    ...
    price: tc.discountPrice && tc.discountPrice < tc.price ? tc.discountPrice : tc.price,
    color: tc.coverColor,
    emoji: tc.coverEmoji,
    tag: tc.isFeatured ? "Featured" : "",
    description: tc.description || defaultDetails.description,
    teacherBio: tc.teacherBio || tc.teacherHeadline || defaultDetails.teacherBio,
    outcomes: tc.outcomes?.length ? tc.outcomes : defaultDetails.outcomes,
    chapters: chaptersList,
    reviews: tc.testimonials?.length ? tc.testimonials.map((t) => ({ name: t.name, avatar: t.avatar, rating: t.rating, comment: t.comment, date: "" })) : [],
    paymentOptions: tc.allowWalletPayment ? ["Wallet Balance", "Credit/Debit Card", "Fawry", "Vodafone Cash"] : ["Credit/Debit Card", "Fawry"],
  } as CourseDetails;
}
