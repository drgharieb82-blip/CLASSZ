...
@router.get("/detail/{plan_id}", response_model=RevisionPlanRead)
...
@router.get("/{student_id}", response_model=list[RevisionPlanRead])
async def list_student_revision_plans(student_id: UUID, session: AsyncSession = Depends(get_db_session)) -> list[RevisionPlanRead]:
    return await service.list_revision_plans(session, student_id)
...
@router.post("/{student_id}", response_model=RevisionPlanRead, status_code=status.HTTP_201_CREATED)
async def create_student_revision_plan(
    student_id: UUID,
    payload: RevisionPlanCreate,
    session: AsyncSession = Depends(get_db_session),
) -> RevisionPlanRead:
    return await service.create_revision_plan(session, payload.model_copy(update={"student_id": student_id}))
...
@router.put("/{plan_id}", response_model=RevisionPlanRead)
async def replace_revision_plan(
    plan_id: UUID,
    payload: RevisionPlanUpdate,
    session: AsyncSession = Depends(get_db_session),
) -> RevisionPlanRead:
