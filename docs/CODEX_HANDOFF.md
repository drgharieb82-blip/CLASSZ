...
- **Student dashboard card overflow pass** (same route) — `AICoachCard`, `AchievementsCard`, and `FavoriteCourseCard` now follow a stricter anti-overflow pattern: `min-w-0`, `overflow-hidden`, responsive grids, wrapped actions, clamped long titles where appropriate, and `min-h` instead of fixed-height content blocks. This pass is intended to be the default pattern for future dashboard cards.
- **Student dashboard ratio tuning** (same route) — `/student` now uses route-level container and grid tuning to fit more proportionally across desktop widths without changing card design, content order, or component architecture. The implementation avoids `transform: scale(...)` and instead relies on a page-local max-width wrapper plus safer desktop column transitions.
